...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt->bind_param('sss', 'certification_content', $content, $content);
    $stmt->execute();
    $stmt->close();
    set_flash('success', 'Certification page updated.');
    redirect_self();
}

$stmt = $conn->prepare("SELECT content FROM page_content WHERE page_key = ?");
$stmt->bind_param("s", "certification_content");
$stmt->execute();
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Edit Certification</h1>
</div>

<form method="POST">
    <div class="mb-3">
        <label class="form-label fw-bold">Certification Information</label>
        <textarea name="content" class="form-control" rows="10"><?= htmlspecialchars($content) ?></textarea>
        <div class="form-text">Explain your certification process and offerings.</div>
    </div>
